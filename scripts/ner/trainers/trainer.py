...
class Trainer(object):
    ...
    def _train_epoch(self, dataloader) -> Dict[str, float]:
        ...
        for i, data in enumerate(dataloader):
          ...
          loss = compute_loss(loss_fn=self.loss_fn, preds=output, labels=labels)
          loss.backward()
          loss = loss.item()

          if self.grad_clip_max_norm != None:
            nn.utils.clip_grad_norm_(parameters=model.parameters(), max_norm=self.grad_clip_max_norm)
          self.optimizer.step()

          labels = labels.to('cpu')
          output = output.to('cpu')
          m = compute_metrics(preds=output, labels=labels, padding_mask=mask, other_ner_tag_idx=self.other_ner_tag_idx, average="weighted",)
          for key in metrics:
            metrics[key].append(np.nan_to_num(m[key])) if key != "loss" else metrics[key].append(loss)
